...
_FREQ_DIR_1 = '../datasets/frequency_stats.csv'
_FREQ_DIR_2 = '../datasets/frequency_stats_TSCC.csv'
...
def compute_stats(fname1, fname2):
    ...
    dict1_filtered = {w: f for w, f in dict1.items() if f > 5}
    dict2_filtered = {w: f for w, f in dict2.items() if f > 5}
    print('len of dict1, dict2:\n', len(dict1), len(dict2))
    print('len of filtered dict1, dict2:\n', len(dict1_filtered), len(dict2_filtered))

    plt.hist(dict2_filtered.values(), bins=100, log=True)
